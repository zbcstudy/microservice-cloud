...
import com.wondertek.springcloud.redisson.operation.RedissonBinary;
import com.wondertek.springcloud.redisson.operation.RedissonCollection;
import com.wondertek.springcloud.redisson.operation.RedissonObject;
import com.wondertek.springcloud.redisson.properties.MultipleServerConfig;
...
public class RedissonConfiguration {
    ...
    public RedissonClient redissonClient() {
        ...
        try {
            ...
            throw new RuntimeException("make Codec object instance was wrong");
        }

        config.setTransportMode(redissonProperties.getTransportMode());

        if (redissonProperties.getThread() != null) {
            config.setThreads(redissonProperties.getThread());
        }
        if (redissonProperties.getNettyThread() != null) {
            config.setNettyThreads(redissonProperties.getNettyThread());
        }
        ...
        switch (redissonProperties.getModel()) {
            case SINGLE:
                ...
                com.wondertek.springcloud.redisson.properties.SingleServerConfig param = redissonProperties.getSingleServerConfig();
